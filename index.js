...
const isValidTime = (a, b, diff) => (a > (b - diff) && a < (b + diff)) || (b > (a - diff) && b < (a + diff));
...
app.post('/api/:gameId', cors(corsOptions), (req, res) => {
    ...
    if (user && requestAgent === agent && hashCode(JSON.stringify(req.body) + tsid) === Number(hash)) {
        const now = Date.now() - diff;
        const { start, end, tt, ts, nick = 'Unknown' } = req.body;

        const calculatedTs = (end - start);

        const validTime = (isValidTime(ts, now, 5000) && isValidTime(tt, calculatedTs, 5));

        client.lpush(gameId, JSON.stringify({ ...req.body, nick: validTime ? nick : `Cheater ${nick}` }), (d) => {
            res.json(d);
        });
    }
    else {
